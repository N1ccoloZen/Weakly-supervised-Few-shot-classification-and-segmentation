...
class ClfSegTransformer(FSCSModule):
    def __init__(self, args):
        ...
        self.backbone = vits.__dict__['vit_small'](patch_size=14)
        ...
        self.sptsize = int(int(args.imgsize // 14) // 4)
    ...
    def forward(self, batch):
        ...
        with torch.no_grad():
            ...
            #print(f"return_qkv is: ", self.sup=='pseudo')

            #print(f"qry_feats type: {type(qry_extr_feats)}")
            #print(f"qry_feats content: {qry_feats}")
            #if hasattr(qry_extr_feats, '__len__'):
                #print(f"qry_feats length: {len(qry_extr_feats)}")
                
            if self.sup == 'pseudo':
                qry_qkv = qry_feats[0]
                qry_feats = qry_feats[1]
                spt_qkv = spt_feats[0]
                spt_feats = spt_feats[1]
                qry_qkv = qry_qkv.repeat_interleave(self.args.way, dim=1)
                
                print("*** qry_qkv is : ", qry_qkv.shape)
                ...
                print(f'This is the shape of generated spt_mask {spt_mask.shape}')
                print(f'This is the shape of qry_mask {qry_mask.shape}')
    ...
    def generate_pseudo_mask(self, qry_qkv, spt_qkv, class_gt, resize=(700, 700), thr=0.4,
