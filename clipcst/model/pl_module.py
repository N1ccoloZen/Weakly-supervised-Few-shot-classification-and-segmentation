...
class FSCSModule(pl.LightningModule, metaclass=abc.ABCMeta):
    ...
    def _shared_step(self, batch, batch_idx, split):
        ...
        '''
        with torch.no_grad():
            class_ids_local = batch['support_classes'].squeeze(1)
            class_ids_local = class_ids_local.clone().detach().to(self.device)
            spt_keys = spt_keys_m.detach()
            spt_keys = F.normalize(spt_keys, dim=-1)

            self.moco_queue.enqueue_from_all_ranks(class_ids_local, spt_keys)
        '''
