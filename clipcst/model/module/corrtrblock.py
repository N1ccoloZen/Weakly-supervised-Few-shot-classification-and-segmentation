...
class CrossAttention(nn.Module):
    ...
    def __init__(self, in_channels, clip_dim, heads=8, groups=4):
        super(CrossAttention, self).__init__()

        self.nheads = heads
        self.in_channels = in_channels
        self.text_projection = nn.Linear(clip_dim, in_channels)
        self.qry = nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, padding=0, bias=True)
        self.key = nn.Linear(in_channels, in_channels, bias=True)
        self.val = nn.Linear(in_channels, in_channels, bias=True)
        self.out = nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, padding=0, bias=True)
