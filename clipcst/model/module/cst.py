...
class CorrelationTransformer(nn.Module):
    ...
    """
    def __init__(self, inch, way, clip_dim):
        super(CorrelationTransformer, self).__init__()
        self.way = way

        def make_building_attentive_block(in_channel, out_channels, kernel_sizes, clip_dim):
            ...
            for idx, (outch, ksz) in enumerate(zip(out_channels, kernel_sizes)):
                ...
                building_block_layers.append(CorrTransformBlock(inch, outch, ksz, stride, clip_dim, padding))
            return nn.ModuleList(building_block_layers)

        self.corrtransformer = make_building_attentive_block(inch[0], [32, 128], [4, 3], clip_dim)
    ...
    def forward(self, headwise_corr, spt_mask, text_embeds):
        
        x = headwise_corr
        for block in self.corrtransformer:
            x, _ = block(x, spt_mask, text_embeds)

        interm_feat = self.linear(x)
