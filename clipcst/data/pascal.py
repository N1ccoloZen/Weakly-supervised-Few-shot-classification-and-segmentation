...
class DatasetPASCAL(Dataset):
    ...
    """
    def __init__(self, datapath, fold, transform, split, way, shot, transforms_aug=[], tokenizer=None):
        ...
        if self.tokenizer is not None:
            self.pre_tokenized_classnames = {cls : self.tokenizer(
                    'a photo of a ' + cls, padding='max_length', max_length=77, truncation=True, return_tensors='pt'
                    ) for cls in self.CLASSES}
            '''
            for fg_class_id in range(0, self.nclass-1):
                fg_name = self.CLASSES[fg_class_id]
                for bk_class_id in list(range(0, self.nclass)) + [21]:

                    if bk_class_id == 21:
                        text = f'a photo of a {fg_name} with a background'
                    else:
                        bk_name = self.CLASSES[bk_class_id]
                        text = f'a photo of a {fg_name} with a background {bk_name}'

                    self.pre_tokenized_classnames[fg_class_id, bk_class_id] = self.tokenizer(
    ...
    def __getitem__(self, idx):
        ...
        classnames = [self.CLASSES[c] for c in _support_classes]
        tokenized_text = torch.cat([self.pre_tokenized_classnames[cls]['input_ids'] for cls in classnames], dim=0)
        '''
        tokenized_text = torch.cat(
            [self.pre_tokenized_classnames[(int(bk), int(fg))]
             for bk, fg in zip(_support_classes, _back_classes)],
             dim=0
        )
        '''
    ...
    def get_all_classes_in_img(self, img_name):
        mask = self.read_mask(img_name)
        classes = torch.unique(mask).tolist()

        classes = [c for c in classes if c not in [0, 255]]
        return classes
    ...
    def sample_episode(self, idx):
        ...
        for sc in support_classes:
            ...
            '''
            all_classes_in_img = self.get_all_classes_in_img(support_names_c[0])

            possible_bck = [c for c in all_classes_in_img if c != sc]
            
            if len(possible_bck) > 0:
                bck_class = np.random.choice(possible_bck, 1)[0]
            else:
                bck_class = 21

            back_classes.append(bck_class)
            '''
        return query_name, support_names, support_classes #, back_classes
